...
def register_user_verification(dp: Dispatcher):
    ...
    async def step_warning(msg: types.Message, state: FSMContext):
        await msg.answer(
            "Верификация документа :\n\n- Фотография паспорта или удостоверения личности со страницей и данными заявки \nФото-пример N1\n\n- Селфи с документом в руках — лицо и данные документа должны быть четко видны \nФото-пример N2",
